...
let command = `mongod --dbpath ${db.data} --logpath ${db.logs} --port ${config.mongo.port} --bind_ip ${config.mongo.host}`;
...
console.log(`${messages.port} ${chalk.cyan.bold(config.mongo.port)}`);
console.log(`${messages.host} ${chalk.cyan.bold(config.mongo.host)}\n`);
